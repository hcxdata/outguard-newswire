...
object Page2VectorDriver {
  def page2Vector(pages: RDD[HttpPage], array: Array[Page2VectorFunction]): RDD[(Int, String, Array[Double])] = {
    val weights: RDD[(Int, String, Array[Double])] = pages.map { page =>
      val vector = array.map(f => f.weight(page))
      (page.id, page.idString, vector)
    }
    weights
  }
}